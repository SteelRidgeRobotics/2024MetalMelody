...
def main():
    ...
    assert detector.addFamily("tag36h11", 3)
    ...
    poseEstConfig = robotpy_apriltag.AprilTagPoseEstimator.Config(
        LimelightConstants.TAGSIZE,
        LimelightConstants.fx,
        LimelightConstants.fy,
        LimelightConstants.cx,
        LimelightConstants.cy,
    )
    ...
    outputStream = CameraServer.putVideo("Detected", 1280, 960)

    # Mats are very memory expensive. Let's reuse these.
    mat = np.zeros((960, 1280, 3), dtype=np.uint8)
    grayMat = np.zeros(shape=(960, 1280), dtype=np.uint8)
