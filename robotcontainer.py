...
class RobotContainer:
    ...
    def __init__(self):
        ...
        JoystickButton(self.functionsController, XboxController.Button.kX).onTrue(self.pivot.runOnce(self.pivot.stow).onlyIf(lambda: self.pivot.getState() is not PivotStates.STOWED).alongWith(self.intake.runOnce(self.intake.stop)))
        JoystickButton(self.functionsController, XboxController.Button.kX).onTrue(self.pivot.runOnce(self.pivot.scoreDownwards).onlyIf(lambda: self.pivot.getState() is PivotStates.STOWED).alongWith(self.intake.runOnce(self.intake.stop)))
